...
class TestParseQuery:
    ...
    def test_parse_query_success(self, adapter: PydanticAdapter) -> None:
        ...
        result = adapter.parse_query(req, UserModel)

        assert isinstance(result, UserModel)
        assert result.name == "Alice"
        assert result.age == 30

    def test_parse_query_validation_error(self, adapter: PydanticAdapter) -> None:
        ...
        with pytest.raises(PydanticValidationError) as exc_info:
            adapter.parse_query(req, UserModel)
...
# Test parse_path
class TestParsePath:
    """Tests for parse_path method."""

    def test_parse_path_success(self, adapter: PydanticAdapter) -> None:
        """Test parsing valid path parameters."""
        from azure.functions import HttpRequest

        req = HttpRequest(
            method="GET",
            url="/api/users/Bob/25",
            route_params={"name": "Bob", "age": "25"},
            body=b"",
        )
        result = adapter.parse_path(req, UserModel)

        assert isinstance(result, UserModel)
        assert result.name == "Bob"
        assert result.age == 25

    def test_parse_path_validation_error(self, adapter: PydanticAdapter) -> None:
        ...
        req = HttpRequest(
            method="GET",
            url="/api/users/Al/25",
            route_params={"name": "Al", "age": "25"},
            body=b"",
        )

        with pytest.raises(PydanticValidationError) as exc_info:
            adapter.parse_path(req, UserModel)
...
class TestParseHeaders:
    """Tests for parse_headers method."""

    def test_parse_headers_success(self, adapter: PydanticAdapter) -> None:
        """Test parsing valid request headers."""
        from azure.functions import HttpRequest

        req = HttpRequest(
            method="GET",
            url="/api/users",
            headers={"name": "Charlie", "age": "35"},
            body=b"",
        )
        result = adapter.parse_headers(req, UserModel)

        assert isinstance(result, UserModel)
        assert result.name == "Charlie"
        assert result.age == 35

    def test_parse_headers_validation_error(self, adapter: PydanticAdapter) -> None:
        """Test validation errors for headers."""
        from azure.functions import HttpRequest
