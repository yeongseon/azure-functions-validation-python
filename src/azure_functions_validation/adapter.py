...
class PydanticAdapter:
    ...
    def parse_body(self, req: HttpRequest, model: Type[BaseModel]) -> Any:
        body = req.get_body()
        if not body:
            # Raise a validation error for a missing body
    ...
    def parse_headers(self, req: HttpRequest, model: Type[BaseModel]) -> Any:
        # Pydantic expects a dict, but headers are a mapping
    ...
    def format_error(self, exc: Exception, loc_prefix: tuple[str, ...]) -> dict[str, Any]:
        if isinstance(exc, PydanticValidationError):
            ...
            return {
                "detail": [{"loc": list(loc_prefix), "msg": str(exc), "type": "internal_error"}]
            }

    def _map_error_type(self, pydantic_type: str) -> str:
        if "missing" in pydantic_type:
            return "missing"
        # For other errors, we can be more transparent and return pydantic's own type
